...
short pairs = 12; //Default number of pairs of devices
...
{
	struct tty0tty_serial *ttyTx = tty->driver_data;
	struct tty0tty_serial *ttyRx = NULL;
	int retval = -EINVAL;
	...
	if((ttyTx->tty->index % 2) == 0)
	{
	  if(tty0tty_table[ttyTx->tty->index+1] != NULL)
	    if (tty0tty_table[ttyTx->tty->index+1]->open_count > 0)
	      ttyx=tty0tty_table[ttyTx->tty->index+1]->tty;
	}
	else
	{
	  if(tty0tty_table[ttyTx->tty->index-1] != NULL)
...
#endif
	  ttyTx->icount.tx += count;
	  ttyRx->icount.rx += count;
	  retval=count;
	}
